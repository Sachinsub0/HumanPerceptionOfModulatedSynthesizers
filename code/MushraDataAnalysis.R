...
mod_test_data %>%
  count(session_uuid, name = "n_modulations") %>%
  filter(n_modulations < 3)

feature_test_data <- mod_avg %>%
...
feature_test_data %>%
  count(session_uuid, name = "n_features") %>%
  filter(n_features < 3)
...
mod_avg_3way <- plot_data_grouped %>%
  mutate(
    rating_score = as.numeric(as.character(rating_score))
  ) %>%
  ...
  summarise(
    mean_rating = mean(rating_score),
    .groups = "drop"
  )

anova_3way <- mod_avg_3way %>%
  anova_test(
    dv = mean_rating,
    wid = session_uuid,
    within = c(modulation, amount_group, feature)
  )
...
problem_ids <- c(
  "204caf3b-55cb-47a9-920b-cc96988bcaa4",
  "8df9dcd9-cfaf-43b3-a14a-c35a6e566444"
)
mod_data_52 <- mod_data %>%
  filter(!session_uuid %in% problem_ids)

# Confirm N = 52
mod_data_52 %>%
  summarise(n_participants = n_distinct(session_uuid))

modulation_means <- mod_data_52 %>%
  group_by(session_uuid, modulation) %>%
  summarise(
    mean_rating = mean(rating_score, na.rm = TRUE),
    .groups = "drop"
...
modulation_means_complete <- modulation_means %>%
  ...
  drop_na(amp, freq, reg) %>%
  pivot_longer(
    cols = c(amp, freq, reg),
    names_to = "modulation",
    values_to = "mean_rating"
  ) %>%
  arrange(session_uuid, modulation)

# Confirm all 52 participants remain
modulation_means_complete %>%
  summarise(n_participants = n_distinct(session_uuid))
...
# Remove the two problematic participants
mod_data_clean <- mod_data %>%
  filter(!session_uuid %in% problem_ids)

# Aggregate to one row per trial/condition
mod_avg_clean <- mod_data_clean %>%
  group_by(
    session_uuid,
    trial_id,
    modulation,
...
# Find participants with all 18 trials
complete_ids <- mod_avg_clean %>%
  count(session_uuid) %>%
  filter(n == 18) %>%
  pull(session_uuid)

length(mod_data_clean)
# ---------------------------------------------------------
...
plot_data_52 <- plot_data %>%
  filter(!session_uuid %in% problem_ids)

plot_data_combined_52 <- plot_data_combined %>%
  filter(!session_uuid %in% problem_ids)
...
plot_data_combined_52 %>%
  summarise(n_participants = n_distinct(session_uuid))

plot_data_grouped <- plot_data_52 %>%
  filter(modulation_amount %in% 1:4) %>%
  mutate(
    amount_group = case_when(
      modulation_amount %in% c(1, 2) ~ "Low",
...
plot_data_lbd %>%
  summarise(
    n_participants = n_distinct(session_uuid)
  )
anova_mod_amount <- anova_test(
  data = plot_data_lbd,
  dv = mean_rating,
  wid = session_uuid,
  within = c(modulation, amount_group)
...
ttest_low_high
plot_data_lbd %>%
  distinct(session_uuid, modulation, amount_group) %>%
  count(session_uuid, modulation) %>%
  filter(n < 2)
...
plot_data_inverted <- plot_data_combined_52 %>%
  filter(modulation_amount %in% 1:4) %>%
  mutate(
    amount_group = case_when(
      modulation_amount %in% c(1, 2) ~ "Low",
