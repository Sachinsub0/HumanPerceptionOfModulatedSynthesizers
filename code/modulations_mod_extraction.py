...
def make_combined_mod_sig(
    n_samples: int, sr: float, freq: float, phase: float, shapes: List[str]
) -> T:
    ...
    if len(corner_indices) > 1:
        for i, idx in enumerate(corner_indices[1:]):
            ...
            curr_shape = util.choice(shapes)
            section = make_mod_signal(
                section_len, section_len, freq=1.0, phase=0.0, shape=curr_shape
            )
...
def _stretch_corners(
    mod_sig: T, top: T, bottom: T, top_val: float = 1.0, bot_val: float = 0.0
) -> T:
    assert mod_sig.ndim == 1
...
def stretch_corners(
    mod_sig: T, max_n_corners: int = 10, smooth_n_frames: int = 32
) -> T:
    assert mod_sig.ndim == 2
...
def check_mod_sig(
    ...
    min_fraction_between_corners: float = 0.10,
) -> bool:
    assert mod_sig.ndim == 1
    ...
    if len(top_indices) > 1:
        distances = [
            top_indices[idx + 1] - loc for idx, loc in enumerate(top_indices[:-1])
        ]
        if min(distances) < min_n_frames:
            return False
    if len(bottom_indices) > 1:
        distances = [
            bottom_indices[idx + 1] - loc for idx, loc in enumerate(bottom_indices[:-1])
        ]
        if min(distances) < min_n_frames:
            return False
    return True
